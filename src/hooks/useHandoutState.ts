...
export const useHandoutState = () => {
  ...
  const [currentHandoutDefinitionKey, setCurrentHandoutDefinitionKey] =
    useState<AllConfigNames>(allConfigs[0].name);

  const currentHandoutConfig =
    ALL_HANDOUT_DEFINITIONS[currentHandoutDefinitionKey];

  const initializeHandoutData = useCallback(
  ...
  const handleDataChange = useCallback(
    (name: string, value: any | null, shouldSpread: boolean = false) => {
  ...
  const changeHandoutType = useCallback(
    (newType: eHandoutDefinitions) => {
